...
export function createOpenAiSemanticEmbeddingClient(
  ...
  return {
    async embed(inputs): Promise<readonly SemanticEmbeddingResult[]> {
      if (inputs.length === 0) {
        return [];
      }

      validateRequests(inputs);
...
function validateRequests(inputs: readonly SemanticEmbeddingRequest[]): void {
  const observedIds = new Set<string>();

  for (const input of inputs) {
    if (input.id.trim().length === 0) {
      throw new Error('Semantic embedding request id must be non-empty.');
    }

    if (observedIds.has(input.id)) {
      throw new Error(`Duplicate semantic embedding request id: ${input.id}`);
    }

    observedIds.add(input.id);

    if (input.text.trim().length === 0) {
      throw new Error(`Semantic embedding text for ${input.id} must be non-empty.`);
    }
  }
}
