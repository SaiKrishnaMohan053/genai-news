...
async function sendMetrics(response: ServerResponse, metrics: AppMetricsRegistry): Promise<void> {
...
export function createWorkerHealthServer({
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && request.url === '/metrics' && metrics) {
      await sendMetrics(response, metrics);
