...
export type DiscoveryFailureSpan = {
  setAttribute(name: string, value: string | boolean | number): unknown;
};
...
export function annotateDiscoveryFailureSpan(
  ...
  span.setAttribute('news.failure.reason', classification.reason);

  span.setAttribute('news.failure.retryable', classification.retryable);
}
